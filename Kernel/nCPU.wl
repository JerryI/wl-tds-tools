...
Print[Style["TDTools`nCPU >> Compiling to C", Italic] ] // Once;

SetAttributes[compiledQ, HoldFirst];
compiledQ /: Set[sym_Symbol, compiledQ[expr_] ] := If[ValueQ[sym], sym, expr];

initialize = compiledQ @ Compile[{{src, _Real, 1}, {meta, _Real, 1}}, Module[{out = Table[0.0, {5 meta[[5]]}]},
  With[{
...
solveNK = compiledQ @ Compile[{{src, _Real, 1}, {dest, _Real, 1}, {meta, _Real, 1}, {iterations, _Integer}}, Module[{
  np = 0.0, kp = 0.0,
  logT = 0.0, ph = 0.0,
...
solveFP = compiledQ @ Compile[{{src, _Real, 1}, {dest, _Real, 1}, {meta, _Real, 1}, {iterations, _Integer}}, Module[{
...
movingAverage = compiledQ @ Compile[{{src, _Real, 1}, {dest, _Real, 1}, {meta, _Real, 1}}, Module[{
  out = Table[0.0, {5 meta[[5]]}]
},
...
saveForDebug = compiledQ @ Compile[{{dest, _Real, 1}, {meta, _Real, 1}}, Module[{
  out = dest
},
...
Print[Style["TDTools`nCPU >> Done", Italic, Background->LightGreen] ] // Once;
